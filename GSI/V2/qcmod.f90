...
! QC_abi
!  Reject because of bust QC
  integer(i_kind),parameter:: ifail_bust_qc=50 
...
!
    ...
    elseif (  obstype == 'hirs2' .or. obstype == 'hirs3' .or. obstype == 'hirs4' .or. & 
              ...
              obstype == 'goes_img' .or. obstype == 'ahi' .or. obstype == 'abi' .or. &
              obstype == 'airs' .or. obstype == 'iasi' .or. &
...
subroutine qc_abi(nchanl,is,ndat,nsig,ich,dplat,luse,   &
     tbc,cldeff_obs,cldeff_fg,id_qc,slons,slats,aivals,errf,varinv,radmod) ! all-sky

!$$$ subprogram documentation block
!               .      .    .
! subprogram:  qc_abi    QC for ABI data of GOES-16
!
!   prgmmr: Chengfeng Feng     org: np23     date: 2020-06-15
!
! input argument list:
!     nchanl       - number of channels per obs
!     ich          - channel number
...
!     tbc          - bias corrected (observed - simulated brightness temperatures)
!     cldeff_obs   - observed cloud effect
!     cldeff_fg    - simulated cloud effect
!     id_qc        - qc index - see qcmod definition
!     errf         - criteria of gross error
!     aivals       - array holding sums for various statistics as a function of obs type
!     varinv       - observation weight (modified obs var error inverse)
...
! Declare passed variables
  ...
  integer(i_kind),dimension(nchanl),intent(inout) :: id_qc
  real(r_kind),dimension(40,ndat),  intent(inout) :: aivals
  real(r_kind),dimension(nchanl),   intent(in   ) :: tbc,cldeff_obs,cldeff_fg
  real(r_kind),dimension(nchanl),   intent(inout) :: errf,varinv
  character(10),                    intent(in   ) :: dplat
  type(rad_obs_type),               intent(in   ) :: radmod

  integer(i_kind) :: i
  real(r_kind)    :: cldeff,slons,slats,term1,term2
...
! Quality control for all-sky
  if (radmod%lcloud_fwd) then

     do i=2,4

        !Bust QC for Channel 8-10
!        cldeff=half*(abs(cldeff_obs(i))+abs(cldeff_fg(i)))

!        if(abs(tbc(i)) > 1.8*cldeff .and. cldeff > cclr(i)) then
!           varinv(i)=zero
!           if(luse) aivals(8+i,is)=aivals(8+i,is) + one
!           if(id_qc(i) == igood_qc) id_qc(i)=ifail_bust_qc
!        end if

        if(slats < 7.5 .or. slats > 342.5 .or. slons < 7.5 .or. slons > 442.5) then
           varinv(i)=zero
           if(luse) aivals(8+i,is)=aivals(8+i,is) + one
           if(id_qc(i) == igood_qc) id_qc(i)=ifail_scanedge_qc
        end if

        if(abs(tbc(i)) > 80.0) then
           varinv(i)=zero
           if(luse) aivals(8+i,is)=aivals(8+i,is) + one
           if(id_qc(i) == igood_qc) id_qc(i)=ifail_bust_qc
        end if
...
!           varinv(i)=zero
!           if(luse) aivals(11+i,is)=aivals(11+i,is) + one
!           if(id_qc(i) == igood_qc) id_qc(i)=ifail_cloud_qc
!        end if
