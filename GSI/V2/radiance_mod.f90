...
!      Alan Geer
!       cldeff=-1.0*half*(cldeff1(i)+cldeff2(i))

!      Use cloud effect of Channel 13 to create the SOEM
       cldeff=half*(abs(cldeff1(7))+abs(cldeff2(7)))
...
!   call ret_amsua(tb_obs,nchanl,tsavg5,zasat,clwp_amsua,ierrret)
  ...
  subroutine radiance_ex_biascor_2(radmod,nchanl,cldeff1,cldeff2,cld_rbc_idx,clr_rbc_idx)
!$$$  subprogram documentation block
!                .      .    .
! subprogram:    radiance_ex_biascor_1
!
...
!$$$ end documentation block
    ...
    integer(i_kind)                   ,intent(in   ) :: nchanl
    real(r_kind),dimension(nchanl)    ,intent(inout) :: cld_rbc_idx
    real(r_kind),dimension(nchanl)    ,intent(inout) :: clr_rbc_idx
...
!      Alan Geer
!       if ((-1.0*cldeff1(i)-cclr(i))*(-1.0*cldeff2(i)-cclr(i))<zero  &
!          .and. abs(cldeff1(i)-cldeff2(i))>=0.1_r_kind) cld_rbc_idx(i)=zero

!      Use Channel 13 to define clear sky
!       if (abs(cldeff1(7))-cclr(i) >= zero .or. abs(cldeff2(7))-cclr(i) >= zero) cld_rbc_idx(i)=zero
       if (abs(cldeff1(7))-cclr(i) >= zero .or. abs(cldeff2(7))-cclr(i) >= zero) clr_rbc_idx(i)=zero
