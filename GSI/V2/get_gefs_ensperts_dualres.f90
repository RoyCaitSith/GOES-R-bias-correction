...
!$$$ end documentation block
  ...
  real(r_kind),pointer,dimension(:,:,:):: ql,qr,qi,qs,qg
  real(r_kind),pointer,dimension(:,:):: ps
...
! create two internal bundles, one on analysis grid and one on ensemble grid
  ...
  call write_spread_dualres(st,vp,tv,rh,oz,cw,ql,qr,qi,qs,qg,ps)

  return
end subroutine ens_spread_dualres


subroutine write_spread_dualres(a,b,c,d,e,f,f1,f2,f3,f4,f5,g2in)
!$$$  subprogram documentation block
!                .      .    .                                       .
...
!$$$ end documentation block
  ...
  real(r_kind),dimension(grd_anl%lat2,grd_anl%lon2,grd_anl%nsig),intent(in):: f1,f2,f3,f4,f5
  ...
  do k=1,grd_anl%nsig
    do j=1,grd_anl%lon2
       do i=1,grd_anl%lat2
         ...
         g3in(i,j,k, 7)=f1(i,j,k)
         g3in(i,j,k, 8)=f2(i,j,k)
         g3in(i,j,k, 9)=f3(i,j,k)
         g3in(i,j,k,10)=f4(i,j,k)
         g3in(i,j,k,11)=f5(i,j,k)
       end do
     end do
  end do
