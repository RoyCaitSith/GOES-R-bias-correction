...
!$$$
  ...
  use qcmod, only: qc_ssmi,qc_seviri,qc_ssu,qc_avhrr,qc_goesimg,qc_abi,qc_msu,qc_irsnd,qc_amsua,qc_mhs,qc_atms
...
! Declare local arrays
  ...
  real(r_kind),dimension(nchanl):: clr_rbc_idx
...
!          emissivity sensitivity bias predictor
           ...
           cp = 0
!            Channel 8 BTs
!           cp = half*(abs(tb_obs(i)-tsim_clr(i))+abs(tsim(i)-tsim_clr(i)))/5.0
!            Channel 13 BTs
           if ((abs(tb_obs(7)-tsim_clr(7))-radmod%cclr(i)) < zero .and. &
               (abs(  tsim(7)-tsim_clr(7))-radmod%cclr(i)) < zero) then
              cp = zero
           else
              cp = half*(abs(tb_obs(7)-tsim_clr(7))+abs(tsim(7)-tsim_clr(7)))/20.0
           end if
           if (n_cp_order == 1 .and. i == 2) then
              pred(2,i) = cp**1.0
           else if (n_cp_order == 4 .and. i == 2) then
              pred(2,i) = cp**1.0
              pred(3,i) = cp**2.0
              pred(6,i) = cp**3.0
              pred(7,i) = cp**4.0
...
!       assign cld_rbc_idx for bias correction in allsky conditions
        ...
        if (radmod%lcloud_fwd .and. radmod%ex_biascor .and. eff_area) then
           ...
           if (radmod%ex_obserr=='ex_obserr2') then     ! comment out for now, need to be tested
              call radiance_ex_biascor(radmod,nchanl,cldeff_obs,cldeff_fg,cld_rbc_idx,clr_rbc_idx)
           end if

!           Channel 13 BTs
           do i=1,nchanl
              if (clr_rbc_idx(i) == 0) then
                 pred(1,i) = zero
                 pred(4,i) = zero
                 pred(5,i) = zero
                 do j=npred-angord+1, npred                                         
...
!
           ...
           call qc_abi(nchanl,is,ndat,nsig,ich,dplat(is),luse(n), &
              tbc,cldeff_obs,cldeff_fg,id_qc,slons,slats,aivals,errf,varinv,radmod)
           ...
           if(iccm > 0)then
              ...
              do ii=1,nchanl
                 ...
                 if (varinv(ii)>tiny_r_kind .and. channel_passive) then
                    ...
                    do k=1,npred
                       my_headm%pred(k,iii)=pred(k,ii)*cld_rbc_idx(ii)*upd_pred(k)
                    end do
...
!              diagbufchan(ipchan_radiag+npred+3,i) = 1.e+10_r_single   ! spread (filled in by EnKF)
              diagbufchan(ipchan_radiag+npred+3,i) = clr_rbc_idx(ich_diag(i))   ! spread (filled in by EnKF)
!              Channel 8 BTs
!              diagbufchan(ipchan_radiag+npred+3+1,i) = cldeff_obs(i)
!              diagbufchan(ipchan_radiag+npred+3+2,i) = cldeff_fg(i) 
!              Channel 13 BTs
              diagbufchan(ipchan_radiag+npred+3+1,i) = tb_obs(7)-tsim_clr(7)
              diagbufchan(ipchan_radiag+npred+3+2,i) = tsim(7)-tsim_clr(7)
              diagbufchan(ipchan_radiag+npred+3+3,i) = tsim_clr(ich_diag(i))

              ioff = ipchan_radiag+npred+3+3
              if (save_jacobian) then
                 j = 1
                 do ii = 1, nvarjac
