...
!$$$ end documentation block
  ...
  use gridmod, only: idsl5,regional,use_gfs_nemsio,use_gfs_ncio,ncepgfs_head,ncepgfs_head,ncepgfs_headv
 ...
 !use hybrid_ensemble_parameters, only: add_bias_perturbation
  ...
  real(r_kind),pointer,dimension(:,:,:) :: clmr=>null()
  real(r_kind),pointer,dimension(:,:,:) :: ramr=>null()
  real(r_kind),pointer,dimension(:,:,:) :: icmr=>null()
  real(r_kind),pointer,dimension(:,:,:) :: snmr=>null()
  real(r_kind),pointer,dimension(:,:,:) :: grmr=>null()
  ...
  real(r_kind),allocatable,dimension(:,:,:)::clbar,rabar,icbar,snbar,grbar
  real(r_kind),allocatable,dimension(:,:)::  pbar_nmmb
  real(r_kind),allocatable,dimension(:,:,:,:)::st_eg,vp_eg,t_eg,rh_eg,oz_eg,cw_eg
  real(r_kind),allocatable,dimension(:,:,:,:)::cl_eg,ra_eg,ic_eg,sn_eg,gr_eg
  ...
  real(r_kind),allocatable,dimension(:,:,:)::clt,rat,ict,snt,grt
  ...
  integer(i_kind) kcl,kra,kic,ksn,kgr
...
!    gfshead%nvcoord is no longer part of NEMSIO header output.
  ...
  ! add netCDF header information
     ...
     if(mype == 0) then
       ...
       write(6,*) ' netCDF: jcap,levs=',jcap_gfs,gfshead%levs
...
! Load reference temperature array (used by general coordinate)
  ...
  inner_vars=1
  num_fields=11*nsig_gfs+2      !  want to transfer u,v,t,q,oz,cw,ps,z from gfs subdomain to slab
...
!  allocate mix ensemble space--horizontal on regional domain, vertical still gefs 
  ...
  allocate(cl_eg(grd_mix%lat2,grd_mix%lon2,grd_mix%nsig,n_ens))
  allocate(ra_eg(grd_mix%lat2,grd_mix%lon2,grd_mix%nsig,n_ens))
  allocate(ic_eg(grd_mix%lat2,grd_mix%lon2,grd_mix%nsig,n_ens))
  allocate(sn_eg(grd_mix%lat2,grd_mix%lon2,grd_mix%nsig,n_ens))
  allocate(gr_eg(grd_mix%lat2,grd_mix%lon2,grd_mix%nsig,n_ens))
  allocate( p_eg_nmmb(grd_mix%lat2,grd_mix%lon2,n_ens))
  st_eg=zero ; vp_eg=zero ;  t_eg=zero ; rh_eg=zero ; oz_eg=zero ; cw_eg=zero
  cl_eg=zero ; ra_eg=zero ; ic_eg=zero ; sn_eg=zero ; gr_eg=zero
  p_eg_nmmb=zero
...
!    allocate necessary space on global grid
     ...
     call gsi_bundlegetpointer(atm_bundle,'ql'  ,clmr,istatus) ; ier = ier + istatus
     call gsi_bundlegetpointer(atm_bundle,'qr'  ,ramr,istatus) ; ier = ier + istatus
     call gsi_bundlegetpointer(atm_bundle,'qi'  ,icmr,istatus) ; ier = ier + istatus
     call gsi_bundlegetpointer(atm_bundle,'qs'  ,snmr,istatus) ; ier = ier + istatus
     call gsi_bundlegetpointer(atm_bundle,'qg'  ,grmr,istatus) ; ier = ier + istatus
     ...
     do k=1,grd_gfs%nsig
        ku=k ; kv=ku+grd_gfs%nsig ; kt=kv+grd_gfs%nsig ; kq=kt+grd_gfs%nsig ; koz=kq+grd_gfs%nsig
        kcw=koz+grd_gfs%nsig ; kcl=kcw+grd_gfs%nsig ; kra=kcl+grd_gfs%nsig ; kic=kra+grd_gfs%nsig
        ksn=kic+grd_gfs%nsig ; kgr=ksn+grd_gfs%nsig
        do j=1,grd_gfs%lon2
           do i=1,grd_gfs%lat2
              ...
              work_sub(1,i,j,kcl)=clmr(i,j,k)
              work_sub(1,i,j,kra)=ramr(i,j,k)
              work_sub(1,i,j,kic)=icmr(i,j,k)
              work_sub(1,i,j,ksn)=snmr(i,j,k)
              work_sub(1,i,j,kgr)=grmr(i,j,k)
           end do
        end do
     end do
...
!    First step is go get sensible temperature and 3d pressure
     ...
     do k=1,grd_mix%nsig
        ...
        kcw=koz+grd_mix%nsig ; kcl=kcw+grd_mix%nsig ; kra=kcl+grd_mix%nsig ; kic=kra+grd_mix%nsig
        ksn=kic+grd_mix%nsig ; kgr=ksn+grd_mix%nsig
        do j=1,grd_mix%lon2
           do i=1,grd_mix%lat2
              ...
              cl_eg(i,j,k,n)=work_sub(1,i,j,kcl)
              ra_eg(i,j,k,n)=work_sub(1,i,j,kra)
              ic_eg(i,j,k,n)=work_sub(1,i,j,kic)
              sn_eg(i,j,k,n)=work_sub(1,i,j,ksn)
              gr_eg(i,j,k,n)=work_sub(1,i,j,kgr)
           end do
        end do
     end do
     kz=num_fields ; kps=kz-1
...
!   next, compute mean of ensembles.
  ...
  allocate(clbar(grd_mix%lat2,grd_mix%lon2,grd_mix%nsig))
  allocate(rabar(grd_mix%lat2,grd_mix%lon2,grd_mix%nsig))
  allocate(icbar(grd_mix%lat2,grd_mix%lon2,grd_mix%nsig))
  allocate(snbar(grd_mix%lat2,grd_mix%lon2,grd_mix%nsig))
  allocate(grbar(grd_mix%lat2,grd_mix%lon2,grd_mix%nsig))
  allocate(pbar_nmmb(grd_mix%lat2,grd_mix%lon2))

!   compute mean state
  stbar=zero ; vpbar=zero ;  tbar=zero ; rhbar=zero ; ozbar=zero ; cwbar=zero
  clbar=zero ; rabar=zero ; icbar=zero ; snbar=zero ; grbar=zero
  pbar_nmmb=zero
  do n=1,n_ens
     do k=1,grd_mix%nsig
        do j=1,grd_mix%lon2
           do i=1,grd_mix%lat2
              ...
              clbar(i,j,k)=clbar(i,j,k)+cl_eg(i,j,k,n)
              rabar(i,j,k)=rabar(i,j,k)+ra_eg(i,j,k,n)
              icbar(i,j,k)=icbar(i,j,k)+ic_eg(i,j,k,n)
              snbar(i,j,k)=snbar(i,j,k)+sn_eg(i,j,k,n)
              grbar(i,j,k)=grbar(i,j,k)+gr_eg(i,j,k,n)
           end do
        end do
     end do
     do j=1,grd_mix%lon2
  ...
  do k=1,grd_mix%nsig
     do j=1,grd_mix%lon2
        do i=1,grd_mix%lat2
           ...
           clbar(i,j,k)=clbar(i,j,k)*bar_norm
           rabar(i,j,k)=rabar(i,j,k)*bar_norm
           icbar(i,j,k)=icbar(i,j,k)*bar_norm
           snbar(i,j,k)=snbar(i,j,k)*bar_norm
           grbar(i,j,k)=grbar(i,j,k)*bar_norm
        end do
     end do
  end do
  do j=1,grd_mix%lon2
...
!www  ensemble perturbation for all but the first member if full_ensemble
     ...
     do k=1,grd_mix%nsig
        do j=1,grd_mix%lon2
           do i=1,grd_mix%lat2
              ...
              cl_eg(i,j,k,n)=cl_eg(i,j,k,n)-clbar(i,j,k)
              ra_eg(i,j,k,n)=ra_eg(i,j,k,n)-rabar(i,j,k)
              ic_eg(i,j,k,n)=ic_eg(i,j,k,n)-icbar(i,j,k)
              sn_eg(i,j,k,n)=sn_eg(i,j,k,n)-snbar(i,j,k)
              gr_eg(i,j,k,n)=gr_eg(i,j,k,n)-grbar(i,j,k)
           end do
        end do
     end do
     do j=1,grd_mix%lon2
  ...
  deallocate(clbar,rabar,icbar,snbar,grbar)
...
!  first need ges_prsl_e, the 3d pressure on the ensemble grid.
  ...
  allocate(clt(grd_ens%lat2,grd_ens%lon2,grd_ens%nsig))
  allocate(rat(grd_ens%lat2,grd_ens%lon2,grd_ens%nsig))
  allocate(ict(grd_ens%lat2,grd_ens%lon2,grd_ens%nsig))
  allocate(snt(grd_ens%lat2,grd_ens%lon2,grd_ens%nsig))
  allocate(grt(grd_ens%lat2,grd_ens%lon2,grd_ens%nsig))
  do n=1,n_ens
     do j=1,grd_ens%lon2
        do i=1,grd_ens%lat2
           do k=1,grd_mix%nsig
