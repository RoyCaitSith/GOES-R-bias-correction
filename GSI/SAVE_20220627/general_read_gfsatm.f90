...
subroutine general_reload(grd,g_z,g_ps,g_tv,g_vor,g_div,g_u,g_v,g_q,g_oz,g_cwmr, &
           g_clmr, g_ramr, g_icmr, g_snmr, g_grmr, &
...
! !OUTPUT PARAMETERS:
  ...
  real(r_kind),dimension(grd%lat2,grd%lon2,grd%nsig),intent(  out) :: g_u,g_v,&
       g_vor,g_div,g_cwmr,g_clmr,g_ramr,g_icmr,g_snmr,g_grmr,g_q,g_oz,g_tv
   ...
   do k=1,icount
      ...
      elseif ( iflag(k) == 15 ) then
         ...
         do j=1,grd%lon2
            do i=1,grd%lat2
               ij=ij+1
               g_grmr(i,j,klev)=sub(ij,k)
            enddo
         enddo
      elseif ( iflag(k) == 16 .and. present(g_cf) ) then  
         klev=ilev(k)
         ij=0
         do j=1,grd%lon2
            do i=1,grd%lat2
...
!$$$
   ...
   call gsi_bundlegetpointer(gfs_bundle,'oz',g_oz  ,ier);istatus=istatus+ier
   call gsi_bundlegetpointer(gfs_bundle,'cw',g_cwmr,ier);istatus=istatus+ier
   call gsi_bundlegetpointer(gfs_bundle,'ql',g_clmr,ier);istatus=istatus+ier
   call gsi_bundlegetpointer(gfs_bundle,'qr',g_ramr,ier);istatus=istatus+ier
   call gsi_bundlegetpointer(gfs_bundle,'qi',g_icmr,ier);istatus=istatus+ier
   call gsi_bundlegetpointer(gfs_bundle,'qs',g_snmr,ier);istatus=istatus+ier
   call gsi_bundlegetpointer(gfs_bundle,'qg',g_grmr,ier);istatus=istatus+ier
   if ( istatus /= 0 ) then
      if ( mype == 0 ) then
         write(6,*) 'general_read_gfsatm_nems: ERROR'
         write(6,*) 'Missing some of the required fields'
   ...
   do k=1,nlevs
      ...
      if (mype==mype_use(icount)) then
         call nemsio_readrecv(gfile,'clwmr','mid layer',k,rwork1d0,iret=iret)
         if (iret /= 0) call error_msg(trim(my_name),trim(filename),'clwmr','read',istop+11,iret)
         if ( diff_res ) then
            grid_b=reshape(rwork1d0,(/size(grid_b,1),size(grid_b,2)/))
            vector(1)=.false.
            call fill2_ns(grid_b,grid_c(:,:,1),latb+2,lonb)
      ...
      if (mype==mype_use(icount)) then
         ...
         if (iret /= 0) call error_msg(trim(my_name),trim(filename),'icmr','read',istop+11,iret)
         if ( diff_res ) then
            grid_b=reshape(rwork1d0,(/size(grid_b,1),size(grid_b,2)/))
            vector(1)=.false.
            call fill2_ns(grid_b,grid_c(:,:,1),latb+2,lonb)
      ...
      if (mype==mype_use(icount)) then
         ...
         if (iret /= 0) call error_msg(trim(my_name),trim(filename),'snmr','read',istop+11,iret)
         if ( diff_res ) then
            grid_b=reshape(rwork1d0,(/size(grid_b,1),size(grid_b,2)/))
            vector(1)=.false.
            call fill2_ns(grid_b,grid_c(:,:,1),latb+2,lonb)
      ...
      if (mype==mype_use(icount)) then
         ...
         if (iret /= 0) call error_msg(trim(my_name),trim(filename),'grle','read',istop+11,iret)
         if ( diff_res ) then
            grid_b=reshape(rwork1d0,(/size(grid_b,1),size(grid_b,2)/))
            vector(1)=.false.
            call fill2_ns(grid_b,grid_c(:,:,1),latb+2,lonb)
...
!                .      .    .
!                .
! subprogram:    general_read_gfsatm_nc  adaptation of read_gfsatm
! for general resolutions
!   prgmmr: martin          org: NCEP/EMC                date:
!   2019-09-25
!
! abstract: copied from read_gfsatm_nems
!
! program history log:
!   2019-09-25  martin     - original; copied from
...
!     sp_a     - structure variable containing spectral information
!     for analysis
!                    (initialized by general_init_spec_vars, located
!                    in general_specmod.f90)
!     sp_b     - structure variable containing spectral information
!     for input
!                     fields
!                    (initialized by general_init_spec_vars, located
!                    in general_specmod.f90)
!     filename - input netcdf file name
!     uvflag   - logical to use u,v (.true.) or st,vp (.false.)
!     perturbations
!     vordivflag - logical to determine if routine should output
!     vorticity and
!                  divergence
!     zflag    - logical to determine if surface height field should
!     be output
