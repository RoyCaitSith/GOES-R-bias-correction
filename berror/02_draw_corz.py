...
pdfname = dir_main + '/Figures/corz.pdf'
...
with PdfPages(pdfname) as pdf:
    ...
    for idv, var in enumerate(variables.keys()):
        ...
        if var == 'q' and qoption == 2:
            temp = np.loadtxt(dir_main + '/Results/corqq.txt').reshape(nsig, mlat)
        else:
            temp = np.loadtxt(dir_main + '/Results/corz_' + var + '.txt').reshape(nsig, mlat)
