...
Exps = ['Before BC, Clear Sky', 'After BC, Clear Sky', 'Before BC, Cloudy Sky', 'After BC, Cloudy Sky', \
        'Before BC, Clear Sky', 'After BC, Clear Sky', 'Before BC, Cloudy Sky', 'After BC, Cloudy Sky', \
        'Before BC, All Sky',   'ASRBC0, All Sky',     'ASRBC1, All Sky',       'ASRBC4, All Sky']
fnames = ['_Clear Sky.txt', '_Clear Sky.txt', '_Cloudy Sky.txt', '_Cloudy Sky.txt', \
          '_Clear Sky.txt', '_Clear Sky.txt', '_Cloudy Sky.txt', '_Cloudy Sky.txt', \
          '_All Sky.txt',   '_All Sky.txt',   '_All Sky.txt',    '_All Sky.txt']
textx = [-3.80, 0.225, 2.5]

schemes = {}
schemes.update({'Tlap (K)':              (['00', '01', '00', '01'],  -4.0,    4.0,  0.2, -60.0, 60.0, 0.2, -4.0,   4.0,  2.00)})
schemes.update({'Scan Angle (rad)':      (['02', '03', '02', '03'], 0.025,  1.225, 0.05, -60.0, 60.0, 0.2,  0.2,  1.20,  0.25)})
...
for datathinning in thingrid:
    for dom in domains:
        for fhours in forecast_hours:
            ...
            pdfname = './Figure_05_Conditional_Bias.pdf'
