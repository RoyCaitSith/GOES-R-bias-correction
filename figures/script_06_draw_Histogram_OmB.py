...
dir_main_1 = dir_GOES + '/24_Revision/BC_coeffs/29_2017051118_CON0_Ch13_Offline_ExBC1_CP0/02_Post_Process/2017051118_06h'
dir_main_2 = dir_GOES + '/24_Revision/BC_coeffs/30_2017051118_CON0_Ch13_Offline_ExBC1_CP1/02_Post_Process/2017051118_06h'
dir_main_3 = dir_GOES + '/24_Revision/BC_coeffs/21_2017051118_CON0_Ch13_Offline_ExBC1_CP4_AR1_TC5/02_Post_Process/2017051118_06h'
...
schemes = {}
schemes.update({'Clear Sky':                           [('Before BC', 'before_BC', 6, 1.5, sns_cmap[0]), ('After BC', 'after_BC', 6, 1.5, sns_cmap[3])]})
schemes.update({'Cloudy Sky, ${\overline{C}} < 55 K$': [('Before BC', 'before_BC', 8, 1.5, sns_cmap[0]), ('After BC', 'after_BC', 8, 1.5, sns_cmap[3])]})
schemes.update({'Cloudy Sky, ${\overline{C}} > 55 K$': [('Before BC', 'before_BC', 9, 1.5, sns_cmap[0]), ('After BC', 'after_BC', 9, 1.5, sns_cmap[3])]})
...
for datathinning in thingrid:
    for dom in domains:
        for fhours in forecast_hours:
            ...
            pdfname = './Figure_06_Histogram_OmB.pdf'
            ...
            with PdfPages(pdfname) as pdf:
                ...
                for idsch, idsch_str in enumerate(schemes.keys()):
                    ...
                    ax.plot([-3, -3], [0, 1000], color='k', ls='--', linewidth=1.0)
                    ax.plot([ 3,  3], [0, 1000], color='k', ls='--', linewidth=1.0)
                    ...
                    ax.set_xticks(np.arange(-12.0, 11.0, 3.0))
                    ...
                    ax.axis([-8.0, 8.0, 0.0001, 100.0])
                    ax.legend(loc='upper right', fontsize=10.0, handlelength=1.50)

                    mtitle = '(' + chr(97+idsch*3) + ') ASRBC0, ' + idsch_str
                ...
                for idsch, idsch_str in enumerate(schemes.keys()):
                    ...
                    mtitle = '(' + chr(97+idsch*3+1) + ') ASRBC1, ' + idsch_str
                    ax.set_title(mtitle, fontsize=10.0, pad=4.0)
                ...
                for idsch, idsch_str in enumerate(schemes.keys()):
                    ...
                    mtitle = '(' + chr(97+idsch*3+2) + ') ASRBC4, ' + idsch_str
                    ax.set_title(mtitle, fontsize=10.0, pad=4.0)
