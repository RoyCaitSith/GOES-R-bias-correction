...
fig, axs = plt.subplots(1, 2, figsize=(8.00, 4.00))
fig.subplots_adjust(left=0.100, bottom=0.075, right=1.000, top=0.975, wspace=0.000, hspace=0.150)
...
x_d01, y_d01 = m(-108.0, 43.5, inverse=False)
x_d02, y_d02 = m(-100.0, 40.0, inverse=False)
x_d03, y_d03 = m( -90.0, 21.5, inverse=False)
x_d04, y_d04 = m( -97.5, 32.5, inverse=False)
...
for idx, (filename, sidx, eidx, zidx, marker, label) in enumerate(zip(filenames, start_idx, end_idx, zero_idx, markers, labels)):
    ...
    ax.legend(loc='upper right', fontsize=10.0, scatterpoints=1, handlelength=1.0)

grade = [20, 33, 63, 82, 95, 112, 125]
cat = ['TD', 'TS', 'Cat1', 'Cat2', 'Cat3', 'Cat4']
clb = plt.colorbar(sc2, ticks=grade, orientation='vertical', pad=0.110, aspect=12.5, shrink=0.95)
clb.set_ticklabels(grade)
clb.set_label('MWS (Knot)', rotation=270, fontsize=10.0)
