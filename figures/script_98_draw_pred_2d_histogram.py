...
dir_main.append(dir_GOES + '/24_Revision/BC_coeffs/03_2017051118_CON0_Ch13_Offline_ExBC1_CP0/02_Post_Process/2017051118_06h')
dir_main.append(dir_GOES + '/24_Revision/BC_coeffs/03_2017051118_CON0_Ch13_Offline_ExBC1_CP0/02_Post_Process/2017051118_06h')
dir_main.append(dir_GOES + '/24_Revision/BC_coeffs/04_2017051118_CON0_Ch13_Offline_ExBC1_CP1/02_Post_Process/2017051118_06h')
dir_main.append(dir_GOES + '/24_Revision/BC_coeffs/05_2017051118_CON0_Ch13_Offline_ExBC1_CP4/02_Post_Process/2017051118_06h')
...
Exps = ['Before BC', 'ASRBC0', 'ASRBC1', 'ASRBC4']
textx = [-1.6, 0.36, 30]

schemes = {}
schemes.update({'Tlap (K)':              (['00', '01', '01', '01'],  -4.0,    4.0,  0.2, -60.0, 60.0, 0.2, -4.0,   4.0,  2.0)})
schemes.update({'Scan Angle (rad)':      (['02', '03', '03', '03'], 0.025,  1.225, 0.05, -60.0, 60.0, 0.2,  0.0,  1.20,  0.3)})
schemes.update({'${\overline{C}}$ (K)':  (['04', '05', '05', '05'],  0.50, 100.50,  1.0, -60.0, 60.0, 0.2,  0.0, 100.0, 20.0)})
...
for datathinning in thingrid:
    for dom in domains:
        for fhours in forecast_hours:
            ...
            pdfname = './Figure_98_Conditional_Bias.pdf'
            ...
            with PdfPages(pdfname) as pdf:
                ...
                for idsch, xvar in enumerate(schemes.keys()):
                    ...
                    for idx, (dmain, exp, sch) in enumerate(zip(dir_main, Exps, scheme)):
                        ...
                        filename = dir_histo + '/' + anl_start_time_str + '_' + anl_end_time_str + '_' + datathinning + '_' + dom + '_' + \
                                   str(fhours).zfill(2) + 'h_channel_' + str(channel).zfill(2) + '_pred_histogram2d_' + sch + '.txt'
                        print(filename)
                        ...
                        mtitle = '(' + chr(97+idsch) + str(idx+1) + ') ' + exp
                        ax.text(textx[idsch], 2.5, mtitle, ha='center', va='center', color='black', fontsize=10.0)
