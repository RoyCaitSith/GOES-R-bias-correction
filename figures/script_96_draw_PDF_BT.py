...
colors   = [sns_cmap[7], sns_cmap[0], sns_cmap[1], sns_cmap[2], sns_cmap[3]]
schemes  = [2, 0, 1, 1, 1]
...
for datathinning in thingrid:
    for dom in domains:
        for fhours in forecast_hours:
            ...
            pdfname = './Figure_96_PDF_BT.pdf'
            with PdfPages(pdfname) as pdf:

                fig_width  = 6.00
                fig_height = 3.00
                fig, axs   = plt.subplots(1, 1, figsize=(fig_width, fig_height))
                fig.subplots_adjust(left=0.125, bottom=0.150, right=0.975, top=0.950, wspace=0.250, hspace=0.450)
                ...
                for (dmain, exp, color, scheme) in zip(dir_main, Exps, colors, schemes):
                    ...
                    filename   = dir_histo + '/' + anl_start_time_str + '_' + anl_end_time_str + '_' + datathinning + '_' + dom + '_' + \
                                 str(fhours).zfill(2) + 'h_channel_' + str(channel).zfill(2) + '_PDF_' + str(scheme).zfill(2) + '.txt'
                    print(filename)
                ...
                #ax.axis([195, 260, 0, 0.20])
                ax.axis([195, 260, 0, 0.07])
