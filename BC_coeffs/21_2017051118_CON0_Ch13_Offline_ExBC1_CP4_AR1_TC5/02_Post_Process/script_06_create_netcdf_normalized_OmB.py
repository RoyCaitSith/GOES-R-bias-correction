...
for datathinning in thingrid:
    for dom in domains:
        for fhours in forecast_hours:
            ...
            var_temp = ncfile.variables['OmB_after_BC'][n_spin_up:, :]
            ...
            ncfileoutput.variables['time'][:] = ncfile.variables['time'][n_spin_up:]
            ...
            for itime in range(0, n_time):
                ...
                OmB_before_BC = ncfile.variables['OmB_before_BC'][itime+n_spin_up, :]
                OmB_after_BC  = ncfile.variables['OmB_after_BC'][itime+n_spin_up, :]
                C_temp        = ncfile.variables['C'][itime+n_spin_up, :]

                index = OmB_after_BC < 66666

                if np.sum(index==True) > 0:

                    ncfileoutput.variables['C'][0, itime, index] = C_temp[index]
                    ncfileoutput.variables['C'][1, itime, index] = C_temp[index]
                    ncfileoutput.variables['normalized_OmB'][0, itime, index] = OmB_before_BC[index]/std_OmB[0, index]
                    ncfileoutput.variables['normalized_OmB'][1, itime, index] = OmB_after_BC[index]/std_OmB[1, index]
                    ...
                    ncfileoutput.variables['observation_error'][1, itime, index] = Cy[index]
                    ncfileoutput.variables['all_sky_scaled_OmB'][0, itime, index] = OmB_before_BC[index]/Cy[index]
                    ncfileoutput.variables['all_sky_scaled_OmB'][1, itime, index] = OmB_after_BC[index]/Cy[index]
