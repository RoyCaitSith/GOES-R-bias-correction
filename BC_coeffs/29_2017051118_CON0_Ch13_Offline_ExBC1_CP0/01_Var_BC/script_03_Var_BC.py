...
dir_main   = dir_GOES + '/24_Revision/BC_coeffs/29_2017051118_CON0_Ch13_Offline_ExBC1_CP0/01_Var_BC'
dir_bufr   = dir_GOES + '/Data/abi'
dir_option = dir_main + '/option_03'
dir_diag   = dir_main + '/save_files'

anl_start_time   = datetime.datetime(2017, 5, 13, 18, 0, 0)
...
for dom in domains:
    for fhours in forecast_hours:
        ...
        time_last_str  = time_now.strftime('%Y%m%d%H') + '_' + str(0).zfill(2)
        ftime_last_str = ftime.strftime('%Y%m%d%H')
        time_last_str  = '2017051312_10'
        ftime_last_str = '2017051318'

        while time_now <= anl_end_time:
            for ict in range(1, cycling_times+1):

                dir_save = dir_diag + '/' + time_last_str

                print('Copy satbias_out to satbias_in')
                diag_satbias = dir_save + "/satbias_out." + ftime_last_str + "." + dom
                os.system('cp ' + diag_satbias + ' ' + dir_option + '/comgsi_satbias_in')

                print('Copy satbias_pc.out to satbias_pc')
                diag_satbias = dir_save + "/satbias_pc.out." + ftime_last_str + "." + dom
                os.system('cp ' + diag_satbias + ' ' + dir_option + '/comgsi_satbias_pc_in')

                time_now_str = time_now.strftime('%Y%m%d%H')
                dir_save     = dir_diag + '/' + time_now_str + '_' + str(ict).zfill(2)
                ...
                time_last_str  = time_now_str + '_' + str(ict).zfill(2)
                ftime_last_str = ftime_str
