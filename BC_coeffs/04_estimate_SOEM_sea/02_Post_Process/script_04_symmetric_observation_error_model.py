...
variables = {'OmB_before_BC': [-74.75, 150.25, 0.5, 'C', '#1f77b4']}
        ...
        for fhours in forecast_hours:
            ...
            for var in variables.keys():
                ...
                for itime in range(0, n_time):
                    ...
                    temp_water = ncfile.variables['water_coverage'][itime, :]
                    temp_land  = ncfile.variables['land_coverage'][itime, :]
                    index      = temp_OmB < 66666
                    OmB        = OmB + list(temp_OmB[index])
                    C          = C + list(temp_C[index])
                    qc         = qc + list(temp_qc[index])
                    water      = water + list(temp_water[index])
                    land       = land + list(temp_land[index])
                ...
                for (ibin, bcenter) in enumerate(bin_center):
                    ...
                    #index = (C >= bcenter - 0.5*range_interval) &  (C < bcenter + 0.5*range_interval) & (qc != -50) & (qc != 50) & (qc != -51) & (qc != 51) & (water == 1.0)
                    #index = (C >= bcenter - 0.5*range_interval) &  (C < bcenter + 0.5*range_interval) & (qc != -50) & (qc != 50) & (qc != -51) & (qc != 51) & (water == 0.0)
