        ...
        for fhours in forecast_hours:
            ...
            for var in variables:
                ...
                for itime in range(0, n_time):
                    ...
                    water_temp = ncfile.variables['water_coverage'][itime, :]
                    land_temp  = ncfile.variables['land_coverage'][itime, :]
                    index      = (var_temp < 66666) & (qc_temp != -50) & (qc_temp != 50) & (qc_temp != -51) & (qc_temp != 51)
                    #index      = (var_temp < 66666) & (qc_temp != -50) & (qc_temp != 50) & (qc_temp != -51) & (qc_temp != 51) & (water_temp == 1.0)
                    #index      = (var_temp < 66666) & (qc_temp != -50) & (qc_temp != 50) & (qc_temp != -51) & (qc_temp != 51) & (water_temp == 0.0)
                ...
                #filename = dir_save + '/' + anl_start_time_str + '_' + anl_end_time_str + '_' + datathinning + '_' + dom + '_' + \
                           #str(fhours).zfill(2) + 'h_channel_' + str(channel).zfill(2) + '_' + var + '_statistics_water.txt'
                #filename = dir_save + '/' + anl_start_time_str + '_' + anl_end_time_str + '_' + datathinning + '_' + dom + '_' + \
                           #str(fhours).zfill(2) + 'h_channel_' + str(channel).zfill(2) + '_' + var + '_statistics_land.txt'
                print(filename)
