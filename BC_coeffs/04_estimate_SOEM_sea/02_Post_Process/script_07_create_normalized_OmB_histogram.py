...
for datathinning in thingrid:
    for dom in domains:
        for fhours in forecast_hours:
            ...
            BT_obs_temp      = [ncfile.variables['BT_obs_before_BC'][:,:]]
            BT_bkg_temp      = [ncfile.variables['BT_bkg'][:,:]]
            OmB_temp         = [ncfile.variables['OmB_before_BC'][:,:]]
            cldeff_obs_temp  = [ncfile.variables['cldeff_obs'][:,:]]
            cldeff_bkg_temp  = [ncfile.variables['cldeff_bkg'][:,:]]
            C_temp           = [ncfile.variables['C'][:,:]]
            id_qc_temp       = [ncfile.variables['id_qc'][:,:]]
            clr_rbc_idx_temp = [ncfile.variables['clr_rbc_idx'][:,:]]
            ncfile.close()
                ...
                for idscheme in range(0, n_scheme):
                    ...
                    for itime in range(0, n_time):
                        ...
                        if idscheme%n_method == 0:
                            ...
                            index = (var_temp < 66666) & (id_qc != -50) & (id_qc != 50) & (id_qc != -51) & (id_qc != 51) & \
                                    (abs(cldeff_obs) < cclr[ids]) & (abs(cldeff_bkg) < cclr[ids])
                            #index = (var_temp < 66666) & (id_qc != -50) & (id_qc != 50) & (id_qc != -51) & (id_qc != 51) & (C > 1.5) & (C < cclr[ids])
                        elif idscheme%n_method == 2:
                            index = (var_temp < 66666) & (id_qc != -50) & (id_qc != 50) & (id_qc != -51) & (id_qc != 51) & \
                                    ((abs(cldeff_obs) >= cclr[ids]) | (abs(cldeff_bkg) >= cclr[ids]))
                            #index = (var_temp < 66666) & (id_qc != -50) & (id_qc != 50) & (id_qc != -51) & (id_qc != 51) & (C < 1.5)
